...
export function ParseYaml(field: string) {
  ...
  const docPipelines = useMemo(() => {
    ...
    const docService = docItems.find(
      (item: any) => item.key.source === "service"
    );
    return docService?.value.items.find(
      (item: any) => item.key.source === "pipelines"
    );
  }, [yamlContent]);
  ...
  const level2Offset = (keyword: string) => {
    return (
      (docPipelines &&
        docPipelines.value &&
        docPipelines.value.items.filter(
          (item: any) => item.key.source === keyword
        )[0] &&
        docPipelines.value.items.filter(
          (item: any) => item.key.source === keyword
        )[0].key.offset) ||
  ...
  const level3Offset = (level2Keyword: string, keyword: string) => {
    return (
      (docPipelines &&
        docPipelines.value.items.filter(
          (item: any) => item.key.source === level2Keyword
        )[0] &&
        docPipelines.value.items
          .filter((item: any) => item.key.source === level2Keyword)[0]
          .value.items.filter((item: any) => item.key.source === keyword)[0].key
          .offset) ||
      0
    );
  };

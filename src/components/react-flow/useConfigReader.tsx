...
const createNode = (
    ...
    const calculateReceiverYposition = (
        receivers: string[],
        index: number,
        parentHeight: number
    ) => {
    ...
    const receiverPosition = (
        ...
        const positionY = calculateReceiverYposition(
            receivers,
            index,
            parentHeight
        )!;
    ...
    const exporterPosition = (
        ...
        const processorLength = processors?.length
            ? processors?.length * 200 + 260
            : 250;
...
const useConfigReader = (
    ...
    useEffect(() => {
        ...
        parentNodeLabels.forEach((parentNodeLabel) => {
            ...
            const max =
                Math.max(receivers, exporters) || receivers || exporters || 1;

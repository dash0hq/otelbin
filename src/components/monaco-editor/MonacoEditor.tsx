...
export default function MonacoEditor({
	...
	return (
		<>
			{isServer ? <WelcomeModal open={openDialog} setOpen={setOpenDialog} /> : <></>}
			<div className="flex h-full flex-col">
              ...
              ${activeView === "both" ? "border-r-[8px]" : "border-r-[0px]"}`}
							style={{
								width:
									activeView === "code" ? "100%" : activeView === "pipeline" ? "0px" : `${width}px`,
							...
							<Editor
								...
								value={
									!clicked
										? (configs &&
												configs?.length > 0 &&
												configs.filter((config) => config.id?.toString() === id)[0]?.config) ||
										  data.config
										: data.config
								}

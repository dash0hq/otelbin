...
export default function ErrorConsole({ errors }: { errors?: IError }) {
    ...
    return isOpenErrorConsole ? (
        <div className="absolute bottom-3 left-2 z-10 h-[20vh] w-[calc(100%-20px)] rounded-md bg-otelbinBlackGrey px-3 pb-1 pt-3">
            <p className="mb-2 text-xs font-bold text-otelbinLightGrey">
                Errors
            </p>
...
export function ErrorCount({
    ...
    function countErrors() {
        ...
        if (errors?.jsYamlError?.mark.line !== null) {
            errorCount =
                (errors && errors.ajvErrors && errors.ajvErrors.length + 1) ||
                0;
            return errorCount;
        } else {
            errorCount =
                (errors && errors.ajvErrors && errors.ajvErrors.length) || 0;
            return errorCount;
        }
    }
    ...
    return (
        <div
            ...
            className={`${
                ...
                !isOpen &&
                "h-[26px] justify-center rounded-md bg-otelbinBlackGrey px-2"
